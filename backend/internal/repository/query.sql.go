...
RETURNING id, full_name, email, email_lookup_hash, phone, phone_lookup_hash, password_hash, token_version, is_adult, role, privacy_notice_version, privacy_notice_accepted_at, privacy_acceptance_hash, crypto_key_version, status, created_at, updated_at, last_login_at, deleted_at, deletion_reason
`
...
SELECT id, full_name, email, email_lookup_hash, phone, phone_lookup_hash, password_hash, token_version, is_adult, role, privacy_notice_version, privacy_notice_accepted_at, privacy_acceptance_hash, crypto_key_version, status, created_at, updated_at, last_login_at, deleted_at, deletion_reason FROM users
WHERE email_lookup_hash = $1 AND deleted_at IS NULL
`

func (q *Queries) GetUserByEmailHash(ctx context.Context, emailLookupHash []byte) (User, error) {
	row := q.db.QueryRowContext(ctx, getUserByEmailHash, emailLookupHash)
	var i User
	err := row.Scan(
		&i.ID,
		&i.FullName,
